...
class RAGAgent:
    ...
    def _compute_schedule_diff(self, old_chunks: list, new_chunks: list) -> tuple:
        ...
        diff_parts.append("")
        diff_parts.append("⚠️ IMPORTANT: This pre-computed diff is generated by an automated Python script using simple row-by-row matching. It CAN contain errors — for example, incorrect key matching, misidentified date formats, false positives from whitespace/formatting differences, or missed changes due to column name variations. YOU MUST cross-check the diff results against the actual raw schedule data provided above. If you find discrepancies between the diff and the raw data, ALWAYS trust the raw data and correct the diff. Report any corrections you make.")
        diff_parts.append("")
...
⚠️ CRITICAL — PRE-COMPUTED DIFF AS STARTING POINT:
A pre-computed diff analysis is included at the end of the retrieved context (marked "PRE-COMPUTED SCHEDULE DIFF"). This diff was generated by an automated Python script that performs simple row-by-row matching using a detected key column (TBS, Id, etc.).

USE IT AS A STARTING POINT — but it CAN contain errors:
- Wrong key detection (e.g., matching on an unstable column)
- Date format parsing mistakes (false positives/negatives)
- Whitespace or formatting-only differences flagged as real changes
- Missed changes due to column name variations between the two schedules
- Incorrect categorization (e.g., a task marked "delayed" that is actually just reformatted)

YOUR RESPONSIBILITY: Cross-check the diff against the actual raw schedule data provided in the context. If you spot discrepancies, ALWAYS trust the raw data and correct the diff's conclusions. If the diff counts and categories look accurate after your verification, use them confidently. If something looks wrong, say so and explain what the correct finding is.

The overall scale of changes (hundreds of differences vs. few) should be reflected accurately in your response — do not minimize genuine large-scale changes, but also do not blindly repeat incorrect diff entries.
